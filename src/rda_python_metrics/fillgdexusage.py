...
class FillGDEXUsage(PgSplit, PgIPInfo):
   ...
   def read_parameters(self):
      ...
      if not opt:
         self.show_usage('fillgdexusage')
      elif 's' not in self.params:
         self.pglog("-s: Missing dataset short name to gather GDEX metrics", self.LGWNEX)
   ...
   #
   def fill_gdex_usages(self, dsids, dranges):
      ...
      for dates in dranges:
         for dsid in dsids:
            ...
            for i in range(pgcnt):
               if (i+1)%20000 == 0:
                  self.pglog("{}/{} GDEX/WEB records processed to add".format(i, wcnt), self.WARNLG)
            ...
            allcnt += pgcnt
            rmsg = self.seconds_to_string_time(tm() - bt)
            self.pglog("{}: {} WEB usage records added for {} GDEX entries in {}".format(strids, awcnt, allcnt, rmsg), self.LOGWRN)
